...
/*
void
repo_putfile::prepareSingleData()
{
...
=======
  std::cout<<"parameters.hasStartBlockId()"<<parameters.hasStartBlockId()<<std::endl;
>>>>>>> 73f85cd88c469ec05166c7729fca0f5d72976c7b
...
repo_putfile::onSingleCheckCommandResponse(const ndn::Interest& interest, const ndn::Data& data)
{
    ...
    if(data.getContent() == (*localData->second).getContent()){
        ...
        process.exec();
             ...
             const auto& content = data.getContent();
             text<<"Data Content is "<<reinterpret_cast<const char*>(content.value());
              process.addMessage(QString::fromStdString(text.str()));
              process.exec();
          }
    m_face.getIoService().stop();
    return;
