...
class NfdStatusHandler(SimpleHTTPRequestHandler):
    ...
    def __serveReport(self):
        ...
        #!! If error occurs when generating report, execute here
        ...
        #!! If no error occurs when generating report, execute here
        else:
            # add stylesheet processing instruction after the XML document type declaration
            # (yes, this is a ugly hack)
            # Insert <?xml-stylesheet type="text/xsl" href="nfd-status.xsl"?> to enable stylesheet
            pos = output.index(">") + 1 #Find first ">" in file
            xml = output[:pos]\
...
def main():
    ...
    parser.add_argument("-f", "--workdir", default="./nfd-status-http-server-files", metavar="DIR",
